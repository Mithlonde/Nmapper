...
function nmapper {
	...
	echo "[+] Method: nmap -p- --min-rate 10000 $IP -oN scans/all-ports.nmap"
	nmap -p- --min-rate 10000 $IP -oN scans/all-ports.nmap && ports=$(cat scans/all-ports.nmap | grep "open" | grep -v "filtered" | cut -d "/" -f 1 | sort -u | xargs | tr ' ' ',')
